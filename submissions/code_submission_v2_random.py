...
def compress(input_data: CompressionInputDataSchema) -> None:
    ...
    if sparsity > 0.01:
        ...
        # Pass array directly to avoid tobytes() copy
        compressed = lzma.compress(arr, preset=1)
    else:
        method = 1 # BZ2 for dense data
        # Level 9 is default but explicit is better
        # Pass array directly to avoid tobytes() copy
        compresslevel = random.randint(5, 9)
        compressed = bz2.compress(arr, compresslevel=compresslevel)
